...
mdns_socket_setup_ipv4(int sock, const struct sockaddr_in* saddr) {
...
#endif
	/**
	 * TTL 是一個整數值，表示數據包在網絡中可以被路由器轉發的最大跳數數量。
	 * 每當數據包通過一個路由器時，TTL 減少 1，當 TTL 達到 0
	 * 時，路由器將丟棄該數據包，以避免無限循環或洪水情況。 ttl =
	 * 1：這表示多播封包的生存時間（TTL）被設定為
	 * 1，封包在傳送過程中只能經過一個路由器，之後就會被丟棄。
	 * 這樣的設定會限制多播封包的傳播範圍只在本地網段內。
	 */
	setsockopt(sock, IPPROTO_IP, IP_MULTICAST_TTL, (const char*)&ttl, sizeof(ttl));
	/**
	 * loopback = 1：這表示允許多播封包在傳送的同時也被發送端自己接收。
	 * 在同一台主機上運行發送端和接收端，接收端可以接收到由自己發送的多播封包。
	 */
	...
	/**
	 * 224.0.0.251
	 * 局部多播地址：在224.0.0.0～224.0.0.255之間，這是為路由協議和其他用途保留的地址，路由器並不轉發屬於此範圍的IP包。
	 */
